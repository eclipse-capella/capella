...
public class RegExpCheckboxTreeViewer extends RegExpTreeViewer {
   ...
   * @param parent_p
   */
  public RegExpCheckboxTreeViewer(Composite parent_p) {
    super(parent_p);
  }
   ...
   */
  @Override
  protected CheckboxTreeViewer doClientViewer(Composite parent_p) {
    ...
    FilteredTree filteredTree = new FilteredTree(parent_p, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER, getFilter()) {
      /**
      ...
      protected TreeViewer doCreateTreeViewer(Composite parent__p, int style_p) {
        return new ContainerCheckedTreeViewer(parent__p, style_p);
      }
    };
