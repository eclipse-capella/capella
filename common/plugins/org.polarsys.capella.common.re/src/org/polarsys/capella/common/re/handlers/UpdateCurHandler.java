...
/**
 */
public class UpdateCurHandler extends AbstractReHandler {
  ...
  public void setEnabled(Object evaluationContext_p) {
    ...
    if (!elements.isEmpty()) {
      for (CatalogElement element : elements) {
        if (!((element.getKind() == CatalogElementKind.REC) || (element.getKind() == CatalogElementKind.REC_RPL) /*|| (element.getKind() == CatalogElementKind.RPL)*/)) {
