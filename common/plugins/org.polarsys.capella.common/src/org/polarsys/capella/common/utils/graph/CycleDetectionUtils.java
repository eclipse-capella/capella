...
public class CycleDetectionUtils {
   ...
   */
  public static <T> boolean containsCycles(IDirectedGraph<T> graph_p) {
    // Get cycles
    List<List<T>> list = getCycles(graph_p);
   ...
   */
  public static <T> List<List<T>> getCycles(IDirectedGraph<T> graph_p) {
    ...
    Iterator<T> nodes = graph_p.getNodes();
    while (nodes.hasNext()) {
      T node = nodes.next();
      Iterator<T> succesors = graph_p.getSucessors(node);
    ...
    List<List<T>> sccSearchResult = sccSearch.findSCC(graph_p, false);
