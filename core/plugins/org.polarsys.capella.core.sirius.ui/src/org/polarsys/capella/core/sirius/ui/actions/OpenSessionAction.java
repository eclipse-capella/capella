...
public class OpenSessionAction extends BaseSelectionListenerAction {
     ...
     */
    public OpenSessionAction(final boolean isDirectUserActionLoading) {
        ...
        for (Object selectedElement : getStructuredSelection().toList()) {
            ...
            try {
                ...
                if (null == session) {
                    ...
                    failedOpeningSessions.put(selectedFile, new Status(IStatus.ERROR, SiriusUIPlugin.getDefault().getPluginId(), "Session can't be opened (null session)")); //$NON-NLS-1$
                    continue;
                }
                ...
                if (!session.isOpen()) {
                    failedOpeningSessions.put(selectedFile, new Status(IStatus.ERROR, SiriusUIPlugin.getDefault().getPluginId(), NLS.bind("Session can't be opened (null session)", selectedFile))); //$NON-NLS-1$
                    continue;
                }
                ...
                SiriusEditPlugin.getPlugin().getUiCallback().askUserAndSaveMigratedSession(session);

                // initialize the corresponding model
                // should be done in a listener but event opened is triggered while the melodymodeller file has not been
                // created
                ...
                //
                // IStatus status =
                // new Status(IStatus.ERROR, SiriusUIPlugin.getDefault().getPluginId(), NLS.bind("An error occured when
                // opening session ({0})", selectedFile), ex); //$NON-NLS-1$
    ...
    public void run() {
        ...
        try {
            ...
            if (shouldRunInProgressService) {
                ...
                // CDO context : if this runnable is not forked, we will get a deadlock on the UI thread in case the
                // user
            ...
            public void run() {
                try {
                    ...
                    if (session.isOpen() || shouldOpenActivityExplorer) {
                        activePage.openEditor(new ActivityExplorerEditorInput(session, SessionHelper.getCapellaProject(session)), ACTIVITY_EXPLORER_EDITOR, true,
