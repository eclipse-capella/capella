    ...
    protected void loadState(IMemento memento_p) {
        ...
        try {
            ...
            if (null == session) {
                ...
                OpenSessionAction openSessionAction = new OpenSessionAction(false);
                // Disable to open the capella dashboard as we are already restoring a Capella Dashboard editor input
                // i.e a CapellaDashboard editor.
                ...
                if (openSessionAction.getFailedOpeningSessions().containsKey(diagramFile)) {
                    ...
                    _sessionStatus = new Status(IStatus.ERROR, SiriusUIPlugin.getDefault().getPluginId(), NLS.bind("Error while opening session {0} [{1}]", firstAnalysisFile, status.getMessage()),
                            status.getException());
                }
            ...
            _sessionStatus = new Status(IStatus.ERROR, SiriusUIPlugin.getDefault().getPluginId(), NLS.bind("Error while opening session {0} [{1}]", firstAnalysisFile, e.getMessage()), e);
