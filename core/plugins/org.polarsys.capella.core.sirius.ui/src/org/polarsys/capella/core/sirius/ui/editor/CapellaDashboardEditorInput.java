    ...
    try {
      ...
      if (null == session) {
        ...
        if (openSessionAction.getFailedOpeningSessions().containsKey(diagramFile)) {
          ...
          _sessionStatus =
              Status.error(NLS.bind("Error while opening session {0} [{1}]", firstAnalysisFile, status.getMessage()), status.getException());
        }
      ...
      _sessionStatus =
          Status.error(NLS.bind("Error while opening session {0} [{1}]", firstAnalysisFile, e.getMessage()), e);
