...
public class CreatePhysicalPathActionHandler extends AbstractDiagramCommandHandler {
  ...
  public Object execute(ExecutionEvent event) throws ExecutionException {
    ...
    if (!edgeEditParts.isEmpty()) {
      ...
      EObject source = PhysicalServices.getService().getPhysicalPathSource(context, diagramElements);
      AbstractReadWriteCommandWithResult customCommand = new AbstractReadWriteCommandWithResult(diagram, context,
          diagramElements, source);
  ...
  private class AbstractReadWriteCommandWithResult extends AbstractReadWriteCommand{
    ...
    public void run() {
      EObject pp = PhysicalServices.getService().createPhysicalPath(context, diagramElements, source);

      String mappingName = MappingConstantsHelper.getMappingPhysicalPath(diagram);

      NodeMapping mapping = DiagramServices.getDiagramServices().getNodeMapping(diagram, mappingName);
      result = DiagramServices.getDiagramServices().createNode(mapping, pp, diagram, diagram);
