...
public class SampleModelWizard extends AbstractExampleInstallerWizard {
                        ...
                        try {
                            ...
                            for (Object folder : structureProvider.getChildren(structureProvider.getRoot())) {
                                for (Object subfile : structureProvider.getChildren(folder)) {
                                    if (subfile instanceof ZipEntry && ((ZipEntry) subfile).getName().endsWith(IProjectDescription.DESCRIPTION_FILE_NAME)) {
                                        BufferedInputStream in = new BufferedInputStream(structureProvider.getContents(subfile));
    ...
    protected void openFiles(IProgressMonitor progressMonitor) {
        ...
        if (!filesToOpen.isEmpty()) {
            ...
            OpenSessionAction action = new OpenSessionAction(false);
                    ...
                    if (zipFile != null) {
                        ...
                        if (root != null) {
                            ...
                            if (child != null && child.size() > 0) {
                                ...
                                importOperation = new ImportOperation(projectDescriptor.getProject().getFullPath(), mainProject, structureProvider, OVERWRITE_ALL_QUERY);
                            }
                        }
                    }
                }
