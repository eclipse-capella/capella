...
public class PhysicalActorItemContribution implements IMDEMenuItemContribution {
  ...
  public Command executionContribution(final EditingDomain editingDomain_p, ModelElement containerElement_p, final ModelElement createdElement_p,
    ...
    if (createdElement_p instanceof PhysicalActor) {
      if (((PartitionableElement) createdElement_p).getRepresentingPartitions().size() == 0) {
        EObject paArch = EcoreUtil2.getFirstContainer(containerElement_p, PaPackage.Literals.PHYSICAL_ARCHITECTURE);
        ...
        final Command createPartCmd =
            CreateChildCommand.create(editingDomain_p, context, new CommandParameter(createdElement_p, CapellacorePackage.Literals.CLASSIFIER__OWNED_FEATURES,
