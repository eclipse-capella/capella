...
public class FunctionalChainEndingFunctions implements IQuery {

    @Override
    public List<Object> compute(Object object) {
        if (isValidInstanceOf(object)) {
            InvolvementGraph graph = new InvolvementGraph((FunctionalChain) object);
            return graph.getNodes().values().stream().filter(n -> graph.isEndingFunction(n)).map(n -> n.getSemantic()).filter(FunctionalChainInvolvementFunction.class::isInstance)
