...
public class OpenRelatedDiagramAction {
  ...
  public void run() {
    ...
    Collection<DRepresentationDescriptor>  representations = DoubleClickBehaviourUtil.INSTANCE.getRepresentationsDescriptors(target);
    if (!representations.isEmpty()) {
      ...
      Collection<RepresentationDescription> descriptions = DialectManager.INSTANCE.getAvailableRepresentationDescriptions(selectedViewpoints, target);
      if (!descriptions.isEmpty()) {
        if (descriptions.size() > 1) {
          new SelectNewRepresentationAction(descriptions, target, currentSession, Messages.OpenRelatedDiagram_Message).run();
        } else {
