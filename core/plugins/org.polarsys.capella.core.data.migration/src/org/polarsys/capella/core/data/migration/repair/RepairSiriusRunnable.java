...
public class RepairSiriusRunnable extends AbstractMigrationRunnable {
	...
	public IStatus run(final MigrationContext context, boolean checkVersion) {
		...
		try {
			LongRunningListenersRegistry.getInstance().operationStarting(
					getClass());
			doRepairMigrate(getName(), getFile(), context.isSkipConfirmation(), context);
		} catch (InterruptedException e) {
			LongRunningListenersRegistry.getInstance().operationAborted(
					getClass());
			...
			LongRunningListenersRegistry.getInstance().operationEnded(
					getClass());
	...
	protected void doRepairMigrate(String actionName, final IFile file,
			final boolean backup, final MigrationContext context)
			throws InterruptedException {
	
		Display.getDefault().syncExec(new Runnable() {
			@Override
			public void run() {
				runRepairMigrate(file, backup, context);
			}
		});

	}

	private void runRepairMigrate(final IFile file, final boolean backup,
			final MigrationContext context) {
			if (file != null) {
				@SuppressWarnings("restriction")
				IStatus validationStatus = new RepresentationFilesRepairValidator()
						.validate(file);
