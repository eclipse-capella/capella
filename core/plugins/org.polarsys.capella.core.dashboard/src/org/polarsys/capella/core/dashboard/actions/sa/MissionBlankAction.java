...
public class MissionBlankAction extends AbstractCapellaAction {
   ...
   * @param session_p
   */
  public MissionBlankAction(Project capellaProject_p, Session session_p) {
    super(Messages.MissionBlankAction_Title, IImageKeys.IMAGE_DESCRIPTOR_NEW_DIAGRAM, capellaProject_p, session_p);
  }
  ...
  protected void doRun(Project capellaProject_p, Session session_p) {
    new AbstractNewDiagramHyperlinkAdapter(session_p) {
      ...
      protected ModelElement getModelElement(Project project_p) {
        return ModelQueryHelper.getSAMissionPkg(project_p);
      }
    }.linkActivated(null);
  }
}
