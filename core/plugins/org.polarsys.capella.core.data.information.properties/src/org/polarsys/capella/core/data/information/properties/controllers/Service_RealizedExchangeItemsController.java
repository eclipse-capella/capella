...
public class Service_RealizedExchangeItemsController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(CapellaElement semanticElement_p) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement_p.eClass(), InformationPackage.Literals.OPERATION__OWNED_EXCHANGE_ITEM_REALIZATIONS);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(CapellaElement semanticElement_p, EStructuralFeature semanticFeature_p, EObject object_p) {
    ...
    for (EObject obj : (List<EObject>) semanticElement_p.eGet(semanticFeature_p)) {
      if ((obj instanceof ExchangeItemRealization)
        && ((ExchangeItemRealization) obj).getTargetElement().equals(object_p))
      {
        linkToRemove = obj;
      }
    }
