...
public class CapellaSearchPage extends DialogPage implements ISearchPage, IReplacePage {
  ...
  public void createControl(Composite parent) {
    ...
    createSearchForControl(composite);
  ...
  private void createSearchForControl(Composite parent) {
    Group searchForSelectionGroup = new Group(parent, SWT.NONE);
    GridDataFactory.fillDefaults().grab(true, true).span(2, 1).applyTo(searchForSelectionGroup);
    ...
    CheckboxFilteredTree filteredTreeViewer1 = new CheckboxFilteredTree(searchForSelectionGroup, SWT.BORDER,
        patternFilter);
    CheckboxFilteredTree filteredTreeViewer2 = new CheckboxFilteredTree(searchForSelectionGroup, SWT.BORDER,
        patternFilter);
  }
  ...
  public boolean performReplace() {
    ...
    if (searchStatus.isOK()) {
      ...
      if (searchResult.getMatchCount() > 0) {
        ...
        capellaReplaceRunnable.setCapellaSearchSettings(capellaSearchSettings);
        ...
        if (capellaSearchSettings.getReplaceTextPattern() != null) {
          ...
          CapellaReplaceHistory.appendSearchSettings(capellaSearchSettings);
        }
      } else {
