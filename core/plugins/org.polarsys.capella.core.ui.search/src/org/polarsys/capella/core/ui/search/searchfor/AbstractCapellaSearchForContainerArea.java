...
public abstract class AbstractCapellaSearchForContainerArea {
	...
	protected Set<EClass> checkedMetaClasses = new HashSet<EClass>();
	protected Set<EClass> displayedMetaClasses = new HashSet<EClass>();
	...
	protected AbstractCapellaSearchForContainerArea oterSideArea;
	...
	protected void createContentArea() {
		...
		filteredTree.getViewer()
				.setContentProvider(partictipantsItemProvider);
		filteredTree.getViewer()
		...
		IStructuredContentProvider contentProvider = (IStructuredContentProvider) filteredTree
	...
	protected PatternFilter createPatternFilter() {
		return new PatternFilter() {
			...
			protected boolean isParentMatch(Viewer viewer, Object element) {
				// Attributes should be displayed when they match or their parent match
				if (element instanceof EAttribute) {// SimpleMode
					return true;
				}
			...
			public Object[] filter(Viewer viewer, Object parent, Object[] elements) {
				...
				if (parent != null) {
					if (!parent.equals("")) {
						...
						displayedMetaClasses.clear();
						List<Object> filteredResults = new ArrayList<Object>();
						for (Object element : result) {
						  if (element instanceof EAttribute) {
								updateDisplayedElements(filter(viewer, element, elements));
                ...
                filteredResults.add(element);
						...
						result = filteredResults.toArray();
					}
				}
				return result;
			}
			...
			public Object[] filter(Viewer viewer, TreePath parentPath, Object[] elements) {
				Object[] result = super.filter(viewer, parentPath, elements);
				return result;
			}
			...
			private void updateDisplayedElements(Object[] elements) {
				for (Object displayedElement : elements) {
					if (displayedElement instanceof EClass) {
						displayedMetaClasses.add((EClass) displayedElement);
					}
				}
			}
		};
	...
	protected void createButtonsArea() {
		...
		GridData rightPaneContainerGridData = new GridData(
	...
	protected SelectionListener getSelectionListener(boolean selected) {
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        try {
          //Must be run in UI thread to have the "show while busy" dialog 
