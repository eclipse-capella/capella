...
import org.polarsys.capella.core.refinement.commands.GenerateCommunicationLinkDelegationsCommand;

/**
 * This action launches the automatic interface delegations generation.
 */
public class GenerateCommunicationLinkDelegationsAction extends AbstractPartToComponentAction implements IObjectActionDelegate {
  private static final String PROGRESS_BAR_NAME = "Communication links delegations processing..."; //$NON-NLS-1$
  ...
  public void run(IAction action) {
    ...
    if (object instanceof LogicalComponent) {
      ...
      if (root == null || root instanceof LogicalComponent) {
        MessageDialog
            .openError(PlatformUI.getWorkbench().getDisplay().getActiveShell(), "Communication link delegations", "Cannot be performed into an inner component"); //$NON-NLS-1$ //$NON-NLS-2$
        return;
      }
    }
