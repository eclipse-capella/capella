...
public class Rule_Execution extends CommonRule {
  ...
  protected boolean transformIsRequired(EObject element_p, ITransfo transfo_p) {
    ...
    InteractionFragment endTarget = execution.getStart();
    
    if (endSource instanceof AbstractEnd && endTarget instanceof AbstractEnd) {
      return isOrWillBeTransformed(endSource, transfo_p) && 
             isOrWillBeTransformed(endTarget, transfo_p);
    }
    return true;
  }
  ...
  public void firstAttach(EObject element_p, ITransfo transfo_p) throws TransfoException {
    TigerRelationshipHelper.attachIemeWithIeme(element_p, getTargetType(), InteractionPackage.Literals.INTERACTION_FRAGMENT, InteractionPackage.Literals.TIME_LAPSE__START, transfo_p);
