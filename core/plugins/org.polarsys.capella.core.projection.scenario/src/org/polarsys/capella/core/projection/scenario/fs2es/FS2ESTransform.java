...
/**
 */
public class FS2ESTransform extends ScenarioTransform {

  /**
   * Tiger context used. This context is declared implementing an extension point.
   */
  private static final String CAPELLA_SCENARIO_RULES = "org.polarsys.capella.core.projection.scenario.fs2es"; //$NON-NLS-1$
  ...
  protected boolean isValidTransitionedScenario(Scenario contextElement_p, Scenario scenario_p) {
    ...
    return ((scenario_p.getKind() == ScenarioKind.DATA_FLOW) || ((scenario_p.getKind() == ScenarioKind.INTERACTION) && !(TransitionHelper.getService()
        .isFunctionalScenario(scenario_p)))) && sourceBlock.equals(targetBlock);
  }
}
