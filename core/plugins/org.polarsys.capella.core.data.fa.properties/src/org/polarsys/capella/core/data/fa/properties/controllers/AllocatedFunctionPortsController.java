...
public class AllocatedFunctionPortsController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(CapellaElement semanticElement_p) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement_p.eClass(), InformationPackage.Literals.PORT__OWNED_PORT_ALLOCATIONS);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(CapellaElement semanticElement_p, EStructuralFeature semanticFeature_p, EObject object_p) {
    ...
    for (EObject obj : (List<EObject>) semanticElement_p.eGet(semanticFeature_p)) {
      if ((obj instanceof PortAllocation)
        && ((PortAllocation) obj).getTargetElement().equals(object_p))
      {
        linkToRemove = obj;
      }
    }
