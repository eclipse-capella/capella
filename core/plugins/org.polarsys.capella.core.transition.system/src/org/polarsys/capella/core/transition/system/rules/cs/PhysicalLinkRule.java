...
public class PhysicalLinkRule extends AbstractCapellaElementRule {
  ...
  protected void retrieveEnds(EObject source_p, List<EObject> result_p, IContext context_p) {
    if (ContextScopeHandlerHelper.getInstance(context_p).contains(ITransitionConstants.SOURCE_SCOPE, source_p, context_p)) {
      ...
      ContextScopeHandlerHelper.getInstance(context_p).addAll(ITransitionConstants.SOURCE_SCOPE, element.getLinkEnds(), context_p);
    }
  }

  protected void premicesExchangeRelated(EObject element_p, ArrayList<IPremise> needed_p) {
    needed_p.addAll(createDefaultPrecedencePremices(element_p, CsPackage.Literals.PHYSICAL_LINK__LINK_ENDS));
  }
