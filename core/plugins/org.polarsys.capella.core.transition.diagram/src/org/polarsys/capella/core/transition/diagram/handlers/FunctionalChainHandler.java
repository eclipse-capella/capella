...
public class FunctionalChainHandler extends AbstractDiagramHandler {
  ...
  public boolean handles(IContext context_p, RepresentationDescription description_p) {
    if (DiagramHelper.getService().isA(description_p, IDiagramNameConstants.OPERATIONAL_PROCESS_DESCRIPTION_DIAGRAM_NAME)) {
      return true;

    } else if (DiagramHelper.getService().isA(description_p, IDiagramNameConstants.FUNCTIONAL_CHAIN_DIAGRAM_NAME)) {
      return true;
  ...
  public boolean backCovers(IContext context_p, RepresentationDescription description_p) {
    if (DiagramHelper.getService().isA(description_p, IDiagramNameConstants.FUNCTIONAL_CHAIN_DIAGRAM_NAME)) {
      return true;
  ...
  public RepresentationDescription getTargetDescription(IContext context_p, Session session_p, RepresentationDescription description_p) {
    ...
    if (service.isA(description_p, IDiagramNameConstants.OPERATIONAL_PROCESS_DESCRIPTION_DIAGRAM_NAME)) {
      return service.getDescription(session_p, IDiagramNameConstants.FUNCTIONAL_CHAIN_DIAGRAM_NAME);

    } else if (service.isA(description_p, IDiagramNameConstants.FUNCTIONAL_CHAIN_DIAGRAM_NAME)) {
  ...
  public String getTargetName(IContext context_p, DRepresentation diagram_p, RepresentationDescription targetDescription_p) {
    ...
    name = name.replace(DiagramNamingConstants.FUNCTIONAL_CHAIN_DIAGRAM_LOGICAL_PREFIX, DiagramNamingConstants.FUNCTIONAL_CHAIN_DIAGRAM_PHYSICAL_PREFIX);
    name = name.replace(DiagramNamingConstants.FUNCTIONAL_CHAIN_DIAGRAM_SYSTEM_PREFIX, DiagramNamingConstants.FUNCTIONAL_CHAIN_DIAGRAM_LOGICAL_PREFIX);
    name = name.replace(DiagramNamingConstants.OPERATIONAL_PROCESS_DESCRIPTION_DIAGRAM_PREFIX, DiagramNamingConstants.FUNCTIONAL_CHAIN_DIAGRAM_SYSTEM_PREFIX);
    return name;
  }
