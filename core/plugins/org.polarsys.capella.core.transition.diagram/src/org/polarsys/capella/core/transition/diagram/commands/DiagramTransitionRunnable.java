...
public class DiagramTransitionRunnable extends AbstractProcessingCommands<DDiagram> {
  ...
  protected IStatus proceedDiagram(final DRepresentation diagram, final IProgressMonitor monitor) {
    ...
    if (!(diagram instanceof DSemanticDecorator)) {
      return Status.error("Invalid source diagram");
    }
    ...
    if ((sourceSemantic == null) || sourceSemantic.eIsProxy()) {
      return Status.error("Diagram with invalid semantic target");
    }

    // Retrieve the current session
    final Session session = DiagramHelper.getService().getSession(diagram);
    if (session == null) {
      return Status.error("Cannot retrieve session from the given diagram");
    }
    ...
    if ((description == null) || description.eIsProxy()) {
      return Status.error("Invalid source diagram (maybe a missing viewpoint)");
    }
    ...
    if (targetDiagram == null) {
      return Status.error("Invalid target diagram");
    }
  ...
  protected IStatus retrieveTargetDiagram(DRepresentation diagram, Session session, EObject sourceSemantic,
    ...
    if ((allocatingDescription == null) || allocatingDescription.eIsProxy()) {
      return (Status.error(NLS.bind("Diagram ''{0}'' is not yet supported by diagram initialization", EObjectExt.getText(diagram))));
    }
    ...
    if (targetSemantic == null) {
      return (Status.error("No semantic element has been found to put the created diagram"));
    }
    ...
    if (targetDiagram == null) {
      return (Status.error("Cannot create the target diagram"));
    }
