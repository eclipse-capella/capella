...
public class PhysicalActor_InheritedActors implements RefactoredBusinessQuery, IBusinessQuery {
   ...
   * @param currentActor_p the current {@link PhysicalActor}
   * @return list of Actors
   */
  private List<CapellaElement> getRule_MQRY_Actor_Inherited_11(SystemEngineering sysEng_p, PhysicalActor currentActor_p) {
    List<CapellaElement> availableElements = new ArrayList<CapellaElement>(1);

    PhysicalArchitecture pa = SystemEngineeringExt.getOwnedPhysicalArchitecture(sysEng_p);
  ...
  public List<CapellaElement> getAvailableElements(CapellaElement element_p) {
    return RefactorDebugger.callAndTestQuery(
        "GetAvailable_PhysicalActor_InheritedActors__Lib", element_p, getOldAvailableElements(element_p), getEClass(), getClass());//$NON-NLS-1$
  }

  @Override
  public List<CapellaElement> getCurrentElements(CapellaElement element_p, boolean onlyGenerated_p) {
    return RefactorDebugger.callAndTestQuery(
        "GetCurrent_PhysicalActor_InheritedActors", element_p, getOldCurrentElements(element_p, false), getEClass(), getClass());//$NON-NLS-1$
  }
}
