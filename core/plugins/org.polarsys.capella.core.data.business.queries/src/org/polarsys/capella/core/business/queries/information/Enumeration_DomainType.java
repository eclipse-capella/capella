 ...
 */
public class Enumeration_DomainType extends CapellaElement_CurrentAndHigherLevelsQuery implements ExtendedBusinessQueryForLib, RefactoredBusinessQuery {
  ...
  public List<CapellaElement> getDataFromLevel(DataPkg dataPkg_p, CapellaElement capellaElement_p) {
    ...
    // all data types avoiding the current one
    returnValue.addAll(CapellaElementsHelperForBusinessQueries.getCapellaElementsInstancesOf(dataPkg_p, DatatypePackage.Literals.DATA_TYPE, capellaElement_p));
  ...
  public List<CapellaElement> getOldCurrentElements(CapellaElement element_p, boolean onlyGenerated_p) {
    ...
    if (element_p instanceof Enumeration) {
      DataType domainType = ((Enumeration) element_p).getDomainType();
      if (null != domainType) {
  ...
  public List<CapellaElement> getAvailableElements(CapellaElement element_p) {
    return RefactorDebugger
        .callAndTestQuery("GetAvailable_Enumeration_DomainType__Lib", element_p, getOldAvailableElements(element_p), getEClass(), getClass());//$NON-NLS-1$
  }

  @Override
  public List<CapellaElement> getCurrentElements(CapellaElement element_p, boolean onlyGenerated_p) {
    return RefactorDebugger.callAndTestQuery("GetCurrent_Enumeration_DomainType", element_p, getOldCurrentElements(element_p, false), getEClass(), getClass());//$NON-NLS-1$
  }
}