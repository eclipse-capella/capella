 ...
 */
public class PhysicalQuantity_MaxValue extends NumericType_AbstractValue implements ExtendedBusinessQueryForLib, RefactoredBusinessQuery {
  ...
  public List<CapellaElement> getOldCurrentElements(CapellaElement element_p, boolean onlyGenerated_p) {
    ...
    if (element_p instanceof PhysicalQuantity) {
      ...
      if (currentPhysicalDimension.getOwnedMaxValue() != null) {
        currentElements.add(currentPhysicalDimension.getOwnedMaxValue());
      }
    }
    return currentElements;
  }
  ...
  public List<CapellaElement> getAvailableElements(CapellaElement element_p) {
    return RefactorDebugger.callAndTestQuery(
        "GetAvailable_NumericType_DefaultValue__Lib", element_p, getOldAvailableElements(element_p), getEClass(), getClass());//$NON-NLS-1$
  }

  @Override
  public List<CapellaElement> getCurrentElements(CapellaElement element_p, boolean onlyGenerated_p) {
    return RefactorDebugger.callAndTestQuery(
        "GetCurrent_PhysicalQuantity_MaxValue", element_p, getOldCurrentElements(element_p, false), getEClass(), getClass());//$NON-NLS-1$
  }
}