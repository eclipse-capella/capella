...
/**
 * <b> This is a copy of org.eclipse.draw2d.GridLayout. The following changes have been added :
 * <ol>
 * <li>Figure with <code>null</code> constraint are not laid out,</li>
 * <li>A new attribute has been added to GridData (FilteringGridData) : keepFixedLocation.</li>
 * </ol>
 * </b><br>
 * Lays out children into a Grid arrangement in which overall aligment and spacing can be configured, as well as specfic layout requirements for the each
 * individual member of the GridLayout. This layout is a Draw2D port of the swt GridLayout. <code>GridLayout</code> has a number of configuration fields, and
 * the Figures it lays out can have an associated layout data object, called <code>GridData</code> (similar to the swt GridData object). The power of
 * <code>GridLayout</code> lies in the ability to configure <code>GridData</code> for each Figure in the layout.
 * <p>
 * The following code creates a container Figure managed by a <code>GridLayout</code> with 2 columns, containing 3 RectangleFigure shapes, the last of which has
 * been given further layout instructions. Note that it is the <code>GridLayout</code> method <code>setConstraint</code> that binds the child
 * <code>Figure</code> to its layout <code>GridData</code> object.
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * The <code>numColumns</code> field is the most important field in a <code>GridLayout</code>. Widgets are laid out in columns from left to right, and a new row
...
public class FilteringGridLayout extends AbstractLayout {
  ...
  /**
   * horizontalSpacing specifies the number of pixels between the right edge of one cell and the left edge of its neighbouring cell to the right. The default
   * value is 5.
   */
  public int horizontalSpacing = 5;

  /**
   * verticalSpacing specifies the number of pixels between the bottom edge of one cell and the top edge of its neighbouring cell underneath. The default value
   * is 5.
   */
   ...
   */
  public FilteringGridLayout(int numColumns, boolean makeColumnsEqualWidth) {
  ...
  FilteringGridData getData(IFigure[][] grid, int row, int column, int rowCount, int columnCount, boolean first) {
    IFigure figure = grid[row][column];
    if (figure != null) {
      FilteringGridData data = (FilteringGridData) getConstraint(figure);
  ...
  Dimension layout(IFigure container, boolean move, int x, int y, int width, int height, boolean flushCache) {
    ...
    List originalChildren = container.getChildren();
    List children = new ArrayList();
    for (Object child : originalChildren) {
      FilteringGridData data = (FilteringGridData) getConstraint((IFigure) child);
      if ((data != null)) {
        children.add(child);
      }
    }
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      FilteringGridData data = (FilteringGridData) getConstraint(child);
      if (data == null) {
        setConstraint(child, data = new FilteringGridData());
      }
      if (flushCache) {
        data.flushCache();
      }
    ...
    for (int j = 0; j < columnCount; j++) {
      for (int i = 0; i < rowCount; i++) {
        FilteringGridData data = getData(grid, i, j, rowCount, columnCount, true);
        if (data != null) {
          int hSpan = Math.max(1, Math.min(data.horizontalSpan, columnCount));
          if (hSpan == 1) {
      ...
      for (int i = 0; i < rowCount; i++) {
        FilteringGridData data = getData(grid, i, j, rowCount, columnCount, false);
        if (data != null) {
          int hSpan = Math.max(1, Math.min(data.horizontalSpan, columnCount));
          if (hSpan > 1) {
          ...
          if (data != null) {
            if (data.heightHint == SWT.DEFAULT) {
              ...
              if (((currentWidth != data.cacheWidth) && (data.horizontalAlignment == SWT.FILL)) || (data.cacheWidth > currentWidth)) {
                int trim = 0;
                /*
                 * // *Note*: Left this in place from SWT // GridLayout. Not sure if Draw2D Borders or // Scrollbars 'trim' will need to be takeninto account.
                 * if (child instanceof Group) { Group g =(Group)child; trim = g.getSize ().x - g.getClientArea ().width; } else if (child instanceof
                 * Scrollable) { Rectangle rect = ((Scrollable) child).computeTrim (0, 0, 0,0); trim = rect.width; } else { trim = child.getBorderWidth () * 2;
                 * }
                 */
                ...
                if (flush == null) {
                  flush = new FilteringGridData[children.size()];
                }
        ...
        for (int j = 0; j < columnCount; j++) {
          ...
          if (data != null) {
            ...
            switch (data.horizontalAlignment) {
              case SWT.CENTER:
              case FilteringGridData.CENTER:
                childX = gridX + Math.max(0, (cellWidth - childWidth) / 2);
              break;
              case SWT.RIGHT:
              case SWT.END:
              case FilteringGridData.END:
  ...
  /**
   * Sets the layout constraint of the given figure. The constraints can only be of type {@link GridData}.
