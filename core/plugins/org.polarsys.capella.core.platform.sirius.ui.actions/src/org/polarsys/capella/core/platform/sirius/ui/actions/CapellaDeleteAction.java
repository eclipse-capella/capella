...
/**
 */
public class CapellaDeleteAction extends DeleteAction implements IPropertyChangeListener {
  /**
   * Execution manager reminder.
   */
   ...
   */
  public CapellaDeleteAction(ExecutionManager executionManager_p) {
    super(executionManager_p.getEditingDomain(), true);
  ...
  public Command createCommand(Collection<?> selection_p) {
    return new CapellaDeleteCommand(_executionManager, selection_p);
  }
  ...
  public boolean updateSelection(IStructuredSelection selection_p) {
    ...
    List<EObject> list = getStructuredSelection().toList();
    ArrayList<EObject> collection = new ArrayList<EObject>(list);
    boolean result = canDelete(collection);
    if (result) {
  ...
  public static boolean canDelete(Collection<?> selectedElement_p) {
    boolean result = true;
    Iterator<?> iterator = selectedElement_p.iterator();
    // Iterate over selected elements.
    while (iterator.hasNext() && result) {
      Object selectedElement = iterator.next();
      // Take into account Capella elements only.
      if (selectedElement instanceof ModelElement) {
        // Get the metaclass.
        result =
            !CapellaActionsActivator.getDefault().isMetaclassProtected(((ModelElement) selectedElement).eClass())
                && !isElementProtected((ModelElement) selectedElement);
      }
    }
    return result;
  }
   ...
   * @return
   */
  protected static boolean isElementProtected(ModelElement elt_p) {
