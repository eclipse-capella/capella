 ...
 */
public class ProgressMonitoringLabelProvider extends
		MDEAdapterFactoryLabelProvider implements IColorProvider {
	/**
	 ...
	 */
	public ProgressMonitoringLabelProvider(TreeViewer viewer,
			int foregroundColorForReferencingElements) {
	...
	public Color getForeground(Object element) {
		...
		if ((input instanceof TreeData)
				&& (((TreeData) input).isValid(element))) {
	...
	public String getText(Object object) {
		...
		return text.replace("%20", ICommonConstants.EMPTY_STRING
				+ ICommonConstants.WHITE_SPACE_CHARACTER); //$NON-NLS-1$
	}
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		if (0 == columnIndex) {
			...
			if (element instanceof CapellaElement) {
				EnumerationPropertyLiteral status = ((CapellaElement) element)
						.getStatus();
				if (null != status) {
			...
			if (element instanceof DRepresentation) {
				...
				DAnnotation dAnnotation = RepresentationHelper.getAnnotation(
						eAnnot, (DRepresentation) element);
				if (dAnnotation != null) {
