...
public class Scenario_RealizedScenariosController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(CapellaElement semanticElement_p) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement_p.eClass(), InteractionPackage.Literals.SCENARIO__OWNED_SCENARIO_REALIZATION);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(CapellaElement semanticElement_p, EStructuralFeature semanticFeature_p, EObject object_p) {
    ...
    for (EObject obj : (List<EObject>) semanticElement_p.eGet(semanticFeature_p)) {
      if ((obj instanceof ScenarioRealization)
        && ((ScenarioRealization) obj).getTargetElement().equals(object_p))
      {
        linkToRemove = obj;
      }
    }
