...
public class SelectionAlgorithms {
	...
	public static List<InterfaceCommunication> getInterfaceCommunications(Component c1, Component c2) {
		...
		for (InterfaceUse interfaceUse : c1.getOwnedInterfaceUses()) {
			usedInterfaces.add(interfaceUse.getUsedInterface());
		}
		for (InterfaceImplementation interfaceImplementation : c2.getOwnedInterfaceImplementations()) {
			implementedInterfaces.add(interfaceImplementation.getImplementedInterface());
		}
	...
	public static List<LinkCommunication> getLinkCommunications(Component c1, Component c2) {
		// calculate function table that give for each items the targeting receiver links 
		...
		for (CommunicationLink link : CommunicationLinkExt.getReceiverCommunicationLink(c2)) {
		...
		for (CommunicationLink senderLink : CommunicationLinkExt.getSenderCommunicationLink(c1)) {
	...
	public static Collection<ExchangeItem> getExchangeItemsVisibleIn(Component component) {
		List<ExchangeItem> items = new ArrayList<ExchangeItem>();
		for (AbstractExchangeItem item : ExchangeItemExt.getAllExchangeItems(component)) {
	...
	public static Interface getTechnicalInterface(Component source, Component target) {
		List<Interface> interfacesByCom = new ArrayList<Interface>(source.getUsedInterfaces());
