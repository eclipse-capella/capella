...
public class SeekAndRemoveUnmergeableElements extends AbstractMergerOperation {
  ...
  public void perform() throws MergeToolException {    
    ...
    if (!_objectsToDelete.isEmpty()) {
      ...
      _logger.warn(
          new EmbeddedMessage(
              MergeMessages.cannotPerformAFullMerge,
              IReportManagerDefaultComponents.REFINEMENT, 
              LinkUtils.getOutgoingLinkTargets(_result, LinkEnum.MERGE_LINK)
          )
      );
      ...
      for (EObject eobject: _objectsToDelete) {
        ...
        if (
            eobject.eClass() == InteractionPackage.Literals.SEQUENCE_MESSAGE
        ) {
          
          _logger.info(
              new EmbeddedMessage(
                  NLS.bind(
                      MergeMessages.objectRemovedDueToLackOfData,
                      new Object[] {eobject.eClass().getName(), ((NamedElement) eobject).getName()}
                  ),
                  IReportManagerDefaultComponents.REFINEMENT,
                  LinkUtils.getOutgoingLinkTargets((TraceableElement) eobject, LinkEnum.MERGE_LINK)
              )
          );
        }
  ...
  public void preOperation() throws MergeToolException {
    ...
    for (TimeLapse tl : executions) {
      ...
      if (null == execution.getCovered() || _objectsToDelete.contains(execution.getStart()) || _objectsToDelete.contains(execution.getFinish())) {
    ...
    List<TimeLapse> stateFragments = TimeLapseHelper.getTimeLapseOfType(_result, InteractionPackage.Literals.STATE_FRAGMENT);

    for (TimeLapse tl : stateFragments) {
      StateFragment stateFragment = (StateFragment) tl;
      if (
          null == stateFragment.getStart() ||
          null == stateFragment.getStart().getCoveredInstanceRoles() ||
          stateFragment.getStart().getCoveredInstanceRoles().isEmpty() ||
          null == stateFragment.getStart().getCoveredInstanceRoles().get(0) ||
          null == stateFragment.getFinish() ||
