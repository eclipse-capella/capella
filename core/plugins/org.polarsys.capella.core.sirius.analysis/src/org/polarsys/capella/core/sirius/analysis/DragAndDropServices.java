...
public class DragAndDropServices {
  ...
  public boolean partitionableElementCanBeDropped(Part semanticObjectToDrop, EObject targetContainerView) {
    ...
    if (targetContainerView instanceof DDiagramElement) {
      for (DDiagramElement anElement : DiagramServices.getDiagramServices().getDiagramElements((DDiagramElement) targetContainerView)) {
    ...
    if (typeTarget != null) {
      type = (PartitionableElement) CsServices.getService().getParentContainer(semanticObjectToDrop);
      ...
      if (!CsServices.getService().isMultipartMode((ModelElement) target)) {
        el.addAll(CapellaServices.getService().getAllAncestors(semanticObjectToDrop));
      }
