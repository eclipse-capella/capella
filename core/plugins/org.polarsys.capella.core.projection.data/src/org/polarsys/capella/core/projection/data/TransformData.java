...
/**
 */
public class TransformData extends AbstractTransform {

  private static final String TRANSFO_CONTEXT = "org.polarsys.capella.core.projection.data"; //$NON-NLS-1$

  public void setContext(EObject context_p) {
    if (context_p instanceof OperationalAnalysis) {
      _context.add(((OperationalAnalysis) context_p).getOwnedDataPkg());
    } else if (context_p instanceof SystemAnalysis) {
      _context.add(((SystemAnalysis) context_p).getOwnedDataPkg());
    } else if (context_p instanceof LogicalArchitecture) {
      _context.add(((LogicalArchitecture) context_p).getOwnedDataPkg());
    } else if (context_p instanceof PhysicalArchitecture) {
      _context.add(((PhysicalArchitecture) context_p).getOwnedDataPkg());
    } else if ((context_p instanceof PropertyValuePkg) || (context_p instanceof PropertyValueGroup) || (context_p instanceof AbstractPropertyValue)) {
      _context.add(context_p);
    } else if ((context_p instanceof DataPkg) || (context_p instanceof DataValue) || (context_p instanceof ExchangeItem)) {
      _context.add(context_p);
    } else if ((context_p instanceof DataType) || (context_p instanceof DataValue) || (context_p instanceof GeneralClass) || (context_p instanceof Exception)
               || (context_p instanceof Message) || (context_p instanceof Signal) || (context_p instanceof Collection)) {
      _context.add(context_p);
    }
  }
