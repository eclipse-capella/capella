pipeline {
<<<<<<< Upstream, based on origin/releng-add-jenkinsfile
...
=======
  ...
  environment {
	...
	CAPELLA_PRODUCT_PATH = "${WORKSPACE}/releng/plugins/org.polarsys.capella.rcp.product/target/products/org.polarsys.capella.rcp.product/win32/win32/x86_64/"
  }
  ...
  stages {
    ...
    stage('Archive to download.eclipse') {
      steps {
		script {		
		    ...
		    deployer.capellaNightlyUpdateSite("${WORKSPACE}/releng/plugins/org.polarsys.capella.rcp.site/target/repository/**", "${deploymentDirName}/org.polarsys.capella.egf.site")
    ...
    stage('Tests: Install Tests on Capella') {
        steps {
        	script {
        		def capella = "${CAPELLA_PRODUCT_PATH}/capella"
        		
        		sh "chmod 755 ${capella}"
        		
        		eclipse.installFeature("${capella}", "http://download.eclipse.org/tools/orbit/downloads/drops/R20130827064939/repository", "org.jsoup")
        		eclipse.installFeature("${capella}", "https://download.eclipse.org/capella/core/updates/nightly/${BUILD_KEY}/org.polarsys.capella.test.site", "org.polarsys.capella.test.feature.feature.group")
...
>>>>>>> 2fb954d [releng] Update Jenkinsfile to use capella library
		    }
        }
    }
