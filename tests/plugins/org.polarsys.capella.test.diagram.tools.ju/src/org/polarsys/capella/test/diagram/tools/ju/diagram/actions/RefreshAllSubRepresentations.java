...
/**
 * Verify that after the "Refresh All Sub Representations" action, the PC related to the diagram's contextual element
 * reappears
 */
public class RefreshAllSubRepresentations extends AbstractDiagramTestCase {
  ...
  private void testRefresh(Session session) {
    GuiActions.refreshAllSubRepresentations(getAirdFileForLoadedModel(getRequiredTestModel()), session, new Runnable() {
      @Override
      public void run() {
        // Do no test?
      }
