 ...
 *       
 *       
 *       Set the transit interfaces option to true before launching the tests
 * 
 * </pre>
 * 
...
public class LCPCBreakdownWithItf extends TopDownTransitionTestCase {
  ...
  private void initSession() {
    ...
    lsInterfacePkg = getObject(ModelLcPc.lsInterfacePkgId);
    logicalInterfacePkg = getObject(ModelLcPc.logicalInterfacePkgId);
   ...
   *         5.  Interfaces and exchange items are not transformed to Physical Layer
   *         6.  Interface1 and Interface2 are projected to Physical Layer under "Physical System/Interface Pkg" in Physical Architecture
   *         7.  Physical Component LC1 implements Interface1 and Interface2 in physical Layer 
   *         8.  Physical Component LC2 uses Interface1 and Interface2 in physical Layer
   * 
   * </pre>
   */
  private void leafLCTransitionTest() {
    ...
    // Get the interface pkg of physical System
    psInterfacePkg = physicalSystem.getOwnedInterfacePkg();
    mustNotBeNull(psInterfacePkg);
    int projectedIntSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedIntSize), projectedIntSize,
        psInterfacePkg.getOwnedInterfaces().size());
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, pf111Join1Exchange.getName(), lf111Join1Exchange.getName()),
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, join1PF11Exchange.getName(), join1LF11Exchange.getName()),
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, physicalConnection1.getName(), logicalConnection1.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalConnection1) == logicalConnection1));

    pc1OFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc1OFP);
    ...
    pc2IFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc2IFP);
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, physicalConnection2.getName(), logicalConnection2.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalConnection2) == logicalConnection2));

    pc11OFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc11OFP);
    ...
    pc12IFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc12IFP);
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, physicalConnection3.getName(), logicalConnection3.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalConnection3) == logicalConnection3));

    pc21OFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc21OFP);
    ...
    pc22IFP = (ComponentPort) ProjectionTestUtils.getAllocatingPort(lc22IFP);
    ...
    physicalInterface1 = ProjectionTestUtils.getAllocatingInterface(logicalInterface1);
    mustNotBeNull(physicalInterface1);
    assertTrue(NLS.bind(Messages.WrongRealization, physicalInterface1.getName(), logicalInterface1.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalInterface1) == logicalInterface1));

    physicalInterface2 = ProjectionTestUtils.getAllocatingInterface(logicalInterface2);
    mustNotBeNull(physicalInterface2);
    assertTrue(NLS.bind(Messages.WrongRealization, physicalInterface2.getName(), logicalInterface2.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalInterface2) == logicalInterface2));

    // Interface Impl and Use
    assertTrue(NLS.bind(Messages.ErrorMessage, pc1.getName(), physicalInterface1.getName()),
        pc1.getImplementedInterfaces().contains(physicalInterface1));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc1.getName(), physicalInterface2.getName()),
        pc1.getImplementedInterfaces().contains(physicalInterface2));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc11.getName(), physicalInterface1.getName()),
        pc11.getImplementedInterfaces().contains(physicalInterface1));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc12.getName(), physicalInterface2.getName()),
        pc12.getImplementedInterfaces().contains(physicalInterface2));

    assertTrue(NLS.bind(Messages.ErrorMessage, pc2.getName(), physicalInterface1.getName()),
        pc2.getUsedInterfaces().contains(physicalInterface1));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc2.getName(), physicalInterface2.getName()),
        pc2.getUsedInterfaces().contains(physicalInterface2));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc21.getName(), physicalInterface1.getName()),
        pc21.getUsedInterfaces().contains(physicalInterface1));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc22.getName(), physicalInterface2.getName()),
        pc22.getUsedInterfaces().contains(physicalInterface2));
  }
  ...
  private PhysicalComponent testAndGetProjectedPC(LogicalComponent logicalComponent_p) {
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, physicalComponent.getName(), logicalComponent_p.getName()),
  ...
  private AbstractFunction testAndGetProjectedPF(AbstractFunction logicalFunction_p) {
    ...
    assertTrue(NLS.bind(Messages.WrongRealization, physicalFunction.getName(), logicalFunction_p.getName()),
  ...
  private void lc1Transition1Test() {
    ...
    // Check the interfaces are projected
    psInterfacePkg = physicalSystem.getOwnedInterfacePkg();
    mustNotBeNull(psInterfacePkg);
    int projectedIntSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedIntSize), projectedIntSize,
    ...
    List<AbstractTrace> outgoing = psInterfacePkg.getOutgoingTraces();
    for (AbstractTrace trace : outgoing) {
      Object tgt = trace.getTargetElement();
      boolean test = ((tgt != null) && (tgt == logicalInterfacePkg)) || (tgt == logicalSystem)
          || (tgt == lsInterfacePkg);
      assertTrue(NLS.bind(Messages.WrongRealization, psInterfacePkg.getName(), logicalInterfacePkg.getName() + "or" //$NON-NLS-1$
          + logicalSystem.getName()), test);
    }

    outgoing = psInterfacePkg.getOutgoingTraces();
    for (AbstractTrace trace : outgoing) {
      Object tgt = trace.getTargetElement();
      boolean test = (tgt == lsInterfacePkg);
      assertTrue(NLS.bind(Messages.WrongRealization, psInterfacePkg.getName(), lsInterfacePkg.getName()), test);
    }
    ...
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), projectedPCSize,
        physicalSystem.getOwnedPhysicalComponents().size());

    projectedPCSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), projectedPCSize,
        pc1.getOwnedPhysicalComponents().size());

    projectedPCSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), projectedPCSize,
        pc2.getOwnedPhysicalComponents().size());

    int projectedPFSize = 3;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPFSize), rootPF.getSubFunctions().size(), projectedPFSize);

    int projectedExchangeSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedExchangeSize), rootPF.getOwnedFunctionalExchanges().size(),
        projectedExchangeSize);
  }
  ...
  private void lc1Transition2Test() {
    ...
    int projectedPCSize = 3;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), physicalSystem.getOwnedPhysicalComponents().size(),
        projectedPCSize);

    projectedPCSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), pc1.getOwnedPhysicalComponents().size(),
        projectedPCSize);

    projectedPCSize = 2;
    assertEquals(NLS.bind(Messages.WrongSize, projectedPCSize), pc2.getOwnedPhysicalComponents().size(),
        projectedPCSize);
   ...
   *                 3.  External Interfaces1 and 2 are projected to Physical Layer under "InterfacePkg/Logical Interfaces" in Physical Architecture\
   *                 4.  PC22 implements External Interface1 and 2 of Physical  layer
   * 
   * </pre>
   */
  private void logicalSystemTransitionTest() {
    ...
    // Interface Impl and Use
    physicalExternalItf1 = ProjectionTestUtils.getAllocatingInterface(logicalExternalItf1);
    mustNotBeNull(physicalExternalItf1);
    assertTrue(NLS.bind(Messages.WrongRealization, physicalExternalItf1.getName(), logicalExternalItf1.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalExternalItf1) == logicalExternalItf1));

    physicalExternalItf2 = ProjectionTestUtils.getAllocatingInterface(logicalExternalItf2);
    mustNotBeNull(physicalExternalItf2);
    assertTrue(NLS.bind(Messages.WrongRealization, physicalExternalItf2.getName(), logicalExternalItf2.getName()),
        (ProjectionTestUtils.getRealizedTargetElement(physicalExternalItf2) == logicalExternalItf2));

    assertTrue(NLS.bind(Messages.ErrorMessage, pc22.getName(), physicalExternalItf1.getName()),
        pc22.getImplementedInterfaces().contains(physicalExternalItf1));
    assertTrue(NLS.bind(Messages.ErrorMessage, pc22.getName(), physicalExternalItf2.getName()),
        pc22.getImplementedInterfaces().contains(physicalExternalItf2));
  }
}
