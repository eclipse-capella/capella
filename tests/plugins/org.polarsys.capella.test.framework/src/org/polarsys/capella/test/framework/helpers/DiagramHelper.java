...
public class DiagramHelper {
  ...
  public static void setPreferenceAutoRefresh(final boolean value_p) {
    ...
    // preferenceStore.setDefault(SiriusUIPreferencesKeys.PREF_AUTO_REFRESH.name(),
    // false);
  ...
  public static void setPrefereneRefreshOnOpening(final boolean value_p) {
    ...
    // preferenceStore.setDefault(SiriusUIPreferencesKeys.PREF_REFRESH_ON_REPRESENTATION_OPENING.name(),
    // false);
  ...
  /**
   * Return the DRepresention with the given diagram name and
   * ID, null otherwise To be used if several diagrams have the same name
   * 
   * @param session_p
   *          the current Session.
   * @param name_p
  ...
  /**
   * Checks if the session given by its AIRD file path
   * contains the given representation (Diagram)
   * 
  ...
  public static void checkDiagramInSession(DRepresentation representation_p, String filepath_p) {
    ...
    // Assert.assertTrue(MessageFormat.format(HelperMessages.diagramNotContainedInSession,
    // representation_p.getName(), file.getName()), containsRepresentation);
  }

  /**
   * Return the DRepresention with the given name and platformSpecificElementId
   * ( ie next.eResource().getURIFragment(next)), null otherwise. Use in case,
   * we have diagrams with the same name in session.
   * 
   * @param session_p
   *          the current Session.
   * @param name_p
   ...
   */
  public static DRepresentation getDRepresentation(final Session session_p, final String name_p,
      final String platformSpecificElementId_p) {
    ...
    while (it.hasNext()) {
      ...
      if (next.getName().equalsIgnoreCase(name_p)
          && next.eResource().getURIFragment(next).equalsIgnoreCase(platformSpecificElementId_p)) {
        result = next;
        break;
      }
    }
  ...
  /**
   * Return the first {@link DDiagramElement} corresponding to the first
   * occurrence found for a given semantic Object, null otherwise
   * 
   * @param diagram_p
   *          the target diagram
  ...
  /**
   * Return the first {@link DDiagramElement} corresponding to the first
   * occurrence found for a given ID, null otherwise
   * 
   * @param diagram_p
   *          the target diagram
   * @param anID
  ...
  /**
   * Return the {@link DDiagramElement} corresponding to the first occurrence
   * found for the given ID and the given Container, null otherwise
   * <p>
   ...
   * @return null if not represented on the diagram
   */
  public static DDiagramElement getOnDiagramByIDandContainer(final DDiagram diagram_p, final String anID,
      EObject container_p) {

    List<DDiagramElement> elements = new LinkedList<DDiagramElement>(diagram_p.getDiagramElements());
    for (DDiagramElement element : elements) {
      if (element.getTarget().eGet(ModellingcorePackage.Literals.MODEL_ELEMENT__ID).equals(anID)
          && element.eContainer().equals(container_p)) {
        return element;
      }
    }
    return null;
  ...
  /**
   * Return the first {@link DDiagramElement} corresponding to the first
   * occurrence found for a given semantic Object and which is not a DEdge, null
   * otherwise
   * 
   * @param diagram_p
   *          the target diagram
  ...
  /**
   * Return the first {@link DDiagramElement} corresponding to the given EClass
   * and to the first occurrence found for a given semantic Object, null
   * otherwise
   * 
   * @param diagram_p
   *          the target diagram
   * @param semanticObject_p
   *          the semantic object to reach
   * @param expectedSemanticMetaClass_p
   *          the expected EClass of the found DDiagramElement
   * @return null if not represented on the diagram
   */
  public static DDiagramElement getDDiagramElementByEClass(final DDiagram diagram_p, final EObject semanticObject_p,
      EClass expectedSemanticMetaClass_p) {
  ...
  /**
   * Return the {@link DDiagramElement} corresponding to the given
   * semanticObject, edge source and edge target
   * 
   * @param diagram_p
   * @param semanticObject_p
   * @param source_p
   ...
   * @return
   */
  public static DDiagramElement getDDiagramElementBySourceAndTarget(final DDiagram diagram_p,
      final EObject semanticObject_p, EObject source_p, EObject target_p) {

    List<DDiagramElement> elements = new LinkedList<DDiagramElement>(diagram_p.getDiagramElements());
    for (DDiagramElement element : elements) {
      if (element instanceof DEdge) {
        DEdge edge = (DEdge) element;
        if ((element.getTarget() == semanticObject_p) && edge.getSourceNode().equals(source_p)
            && edge.getTargetNode().equals(target_p)) {
          return element;
        }
      }
    }
  ...
  /**
   * return the list of available {@link RepresentationDescription} on which a
   * given semantic element can be created.
   * 
   * @param session_p
   * @param semanticElement_p
   * @return
   */
  public static List<RepresentationDescription> getMatchingRepresentationDescription(final Session session_p,
      final EObject semanticElement_p) {
   ...
   * @param semanticElement_p
   * @param repDescName_p
   * @return null whether no matching {@link RepresentationDescription} are
   *         found
   */
  public static RepresentationDescription getMatchingRepresentationDescription(final Session session_p,
      final EObject semanticElement_p, final String repDescName_p) {
   ...
   * 
   * @param diagram_p
   * @return true if operation is ok (i.e. no exception trapped), false
   *         otherwise
   */
    ...
    final AbstractCommand cmd = new AbstractNonDirtyingCommand() {
      ...
      public void run() {
        ...
        if (shouldAddLayer) {
          ...
          // Cannot rely on equals an hashcode methods, since layer
          // can be
          // "proxied" (not really that concept, but result is the
          // same :
          // equals and hashcode cannot be used to remove element in
          // this
          // list)
          ...
          while (iterator.hasNext() && !removed) {
            ...
            if (EqualityHelper.areEquals(targetLayer_p, layerToRemove)) {
              iterator.remove();
              // Only one layer to remove in that list event if it
              // is
              // a list, not a set
              removed = true;
            }
   ...
   */
  public static List<Layer> getContributedLayers(DiagramDescription diagramDescription_p,
      Collection<Viewpoint> viewpoints_p) {
    return new DiagramComponentizationManager().getAllLayers(viewpoints_p, diagramDescription_p);
    // return new ArrayList(diagramDescription_p.getAllLayers());
  }
  ...
  public static IEditorPart opendiagramEditor(Session session_p, DRepresentation drep_p) {
    ...
    IEditorPart iep = DialectUIManager.INSTANCE.openEditor(session_p, drep_p, new NullProgressMonitor());

    String lbl = ICommonConstants.EMPTY_STRING;

    if (iep == null) {

      EClass eclass = drep_p.eClass();

      if (eclass == DiagramPackage.Literals.DDIAGRAM) {
        lbl = ((DDiagram) drep_p).getDescription().getName();
      } else if (eclass == TablePackage.Literals.DTABLE) {
        lbl = ((DTable) drep_p).getDescription().getName();
      } else if (eclass == DiagramPackage.Literals.DSEMANTIC_DIAGRAM) {
        lbl = ((DSemanticDiagram) drep_p).getDescription().getName();
      }
    }

    // Assert.assertNotNull(NLS.bind(HelperMessages.failToOpenDDiagram, new
    // Object[] { drep_p.getName(), lbl }), iep);
  ...
  public static void checkIsSynchronized(DDiagram diagram_p) {
    ...
    Assert.assertTrue(MessageFormat.format(HelperMessages.failedIsSynchronized, diagram_p.getName()), isSynchronized);
  }
  ...
  public static void checkIsUnSynchronized(DDiagram diagram_p) {
    ...
    Assert.assertTrue(MessageFormat.format(HelperMessages.failedIsUnSynchronized, diagram_p.getName()),
        isUnSynchronized);
  }
  ...
  /**
   * Get the invisible elements of a diagram (hidden and not visible diagram
   * elements)
   * 
   * @param diagram
   *          a given DDiagram
  ...
  /**
   * check if the given contextual elements of a diagram are the same than the
   * calculated ones
   * 
   * @param diagram_p
   *          , the diagram to test
   * @param expectedContextualElementsList_p
  ...
  public static void checkDiagramContextualElements(DDiagram diagram_p, List<EObject> expectedContextualElementsList_p) {
    ...
    Assert.assertTrue(MessageFormat.format(HelperMessages.noContextualElement, diagram_p.getName()),
        hasContextualElements);
    ...
    Assert.assertTrue(
        MessageFormat.format(HelperMessages.wrongNumberOfContextualElement, diagram_p.getName(),
            contextualElements.size(), expectedNumberOfContextualElements), sameSize);
    boolean sameElements = contextualElements.containsAll(expectedContextualElementsList_p)
        && expectedContextualElementsList_p.containsAll(contextualElements);
    Assert.assertTrue(HelperMessages.wrongContextualElement, sameElements);
  }
}
