...
public class CheckDiffPolicyTestCase extends BasicTestCaseWithoutTestModel {
  ...
  public void test() throws Exception {
    ...
    for (String projectName : projectNames) {
      ...
      ModelProviderHelper.getInstance().getModelProvider()
          .requireTestModel(Collections.singletonList(projectName), this);
      IProject eclipseProject = AbstractProvider.getEclipseProjectForTestModel(projectName, this);
      IFile melodyModellerFile = eclipseProject.getFile(projectName
          + "." + CapellaResourceHelper.CAPELLA_MODEL_FILE_EXTENSION); //$NON-NLS-1$
      ...
      assertFalse(
          "several objects in project " + projectName + " have the same ID according to the diff policy used for oracle. Diff policy for oracle must be verified and other test cases can not be trust", errorDetected); //$NON-NLS-1$ //$NON-NLS-2$
      // launch the diff
      ...
      assertFalse(
          "iso comparison found differences. Diff policy for oracle must be verified and other test cases can not be trust", diffsDetected); //$NON-NLS-1$			
    }
  }
