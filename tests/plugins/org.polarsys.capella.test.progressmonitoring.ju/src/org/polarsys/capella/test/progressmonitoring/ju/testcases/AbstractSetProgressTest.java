...
public abstract class AbstractSetProgressTest extends BasicTestCase {
  ...
  protected void initializeModelElements(){
    ...
    booleanProperty = (BooleanPropertyValue) IdManager.getInstance().getEObject(BOOLEANPROPERTYVALUE_BOOLEANPROPERTY_ID, scope);
    assertNotNull(booleanProperty);
    
    enumerationProperty = (EnumerationPropertyValue) IdManager.getInstance().getEObject(ENUMERATIONPROPERTYVALUE_ENUMPROPERTY_ID, scope);
  ...
  protected SetProgressRunSetup createRunSetup(EnumerationPropertyLiteral propertyLiteral, boolean propagateToAllElements, boolean propagateToRepresentations) {
  ...
  protected EnumerationPropertyLiteral getDraftPropertyLiteral(EObject element){
    EnumerationPropertyType ept = CapellaProjectHelper.getEnumerationPropertyType(element, CapellaProjectHelper.PROGRESS_STATUS_KEYWORD);
    assertNotNull(ept);
